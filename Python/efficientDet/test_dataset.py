...
def get_args():
    ...
    parser.add_argument("--data_path", type=str, default=r"C:\Users\jonas\Documents\GitHub\efficientDet\data\COCO\Val", help="the root folder of dataset")
...
def test(opt):
    ...
    for index in range(len(dataset)):
        ...
        # Check if the image is None (indicating an issue loading the image)
        if data is None:
            continue
        ...
        if boxes.shape[0] > 0:
            ...
            for box_id in range(boxes.shape[0]):
                ...
                text_size = cv2.getTextSize(COCO_CLASSES[pred_label] + ' : %.2f' % pred_prob, cv2.FONT_HERSHEY_PLAIN, 1,
                                            1)[0]
